...
Till detta måste man också tänka på att webbservern kommer att fördröja uppkopplingen. Tabellen ovan innebär att det för en besökare med första generationen av 3G-uppkoppling tar nästan en sekund innan den kan ta emot en fil. ƒr det då många filer finns det både begränsningar på hur många anslutningar som kan återanvändas och vad överföringshastigheten mäktar med.
...
Många använder YUI Compressor eller Firefix-tillägget YSlow för detta ñ se [YUI compressor](http://developer.yahoo.com/yui/compressor/ ) för mer information.
...
Som vägledning i valet är att göra som majoriteten av webbplatser som besökaren rimligtvis nyligen besökt ñ det ökar chansen för att filen redan är i webbläsarens cache och inte behöver laddas ner.
...
YUI Compressor eller Firefix-tillägget YSlow kan hjälpa till med att skapa en sådan fil ñ se [YUI compressor](http://developer.yahoo.com/yui/compressor/ ) för mer information.
...
Den första kan fungera om den tekniska lösningen innehåller en effektiv cache, annars lägger man på onödig väntetid innan bilden kan skickas. Tvåan och trean är aldrig en bra idÈ då det innebär att en onödigt trögladdad bild skickas till varje besökare.
...
Ett bra verktyg för detta är Yahoos Smush.it som lämpligen körs via Firefox-tillägget YSlow ñ där kollar den alla bilder som används på en undersida.
...
[://addons.mozilla.org/en-US/firefox/addon/yslow/ YSlow](https) är ett tillägg till webbläsaren Firefox. Med YSlow får man en översikt för merparten av de tekniska aspekterna på en webbplats genom ett betyg på A ñ F per problemområde.
